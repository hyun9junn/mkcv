...
export const KNOWN_KEYS         = new Set(SECTION_CATALOG.map(s => s.key));
export const VALID_DENSITY      = ['comfortable', 'balanced', 'compact'];
export const VALID_FONT         = ['small', 'normal', 'large'];
export const VALID_TPL          = [
  'ats-signal',
  'boardroom',
  'chancellor',
  'classic',
...
export const PERSONAL_FIELD_CATALOG = [
...
export const LINK_FIELDS         = new Set(PERSONAL_FIELD_CATALOG.filter(f => f.isLink).map(f => f.key));
export const KNOWN_PERSONAL_KEYS = new Set(PERSONAL_FIELD_CATALOG.map(f => f.key));
...
export function normalizePersonalFields(rawFields) {
...
export function parseSettings(yaml) {
