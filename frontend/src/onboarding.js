// Onboarding wizard — multi-step guide shown on first visit. Phase 2: converted
// from IIFE-on-window to ESM. The aggregate `onboarding` export preserves the
// shape `window.onboarding` previously had so still-IIFE callers continue to
// work via the compat shim in main.js.
...
function _t(obj) { return obj[app.state.lang] ?? obj.ko; }
...
function _render() {
  ...
  _el('ob-body').innerHTML = `
    <div class="ob-step-header">
      ...
      <span class="ob-title">${_t(s.title)}</span>
    </div>
    <p class="ob-desc">${_t(s.desc)}</p>
    ${s.tips.length ? `<div class="ob-tips">${s.tips.map(tip =>
      `<div class="ob-tip"><span class="ob-tip-label">${tip.label}</span>${_t(tip)}</div>`
    ).join('')}</div>` : ''}
  `;
  ...
  _el('ob-dots').querySelectorAll('.ob-dot').forEach(btn => {
    btn.addEventListener('click', () => { _step = +btn.dataset.step; _render(); });
  });
...
export function initOnboarding() {
  ...
  _el('ob-btn-next').addEventListener('click', () => {
    if (_step < STEPS.length - 1) { _step++; _render(); } else { hide(); }
  });
...
// Test-only hook — reset module-private step counter
export function _resetOnboardingForTesting() {
  _step = 0;
}

export const onboarding = { init: initOnboarding, show, hide };
